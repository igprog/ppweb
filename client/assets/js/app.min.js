angular.module('app', ['ui.router', 'pascalprecht.translate', 'chart.js', 'ngStorage', 'functions', 'charts']).config(['$stateProvider', '$urlRouterProvider', '$translateProvider', '$translatePartialLoaderProvider', '$httpProvider', function ($stateProvider, $urlRouterProvider, $translateProvider, $translatePartialLoaderProvider, $httpProvider) {
    ...
    var webService = 'WeeklyMenus'; $scope.type = 0; $scope.limit = 20; $scope.printType = 1; $scope.currMenu = null; $scope.loadMore = function () { $scope.limit += 20; }
    var load = function () { $scope.loading = true; functions.post(webService, 'LoadClientMenus', { userId: $scope.userId, clientId: $scope.clientId, lang: $scope.config.language }).then(function (d) { $scope.d = d; $scope.loading = false; }); }
    load(); $scope.creatingPdf = false; $scope.pageSizes = ['A4', 'A3', 'A2', 'A1']; var printWeeklyMenu = function (weeklyMenu, printSettings) { $http({ url: '../PrintPdf.asmx/WeeklyMenuPdf', method: "POST", data: { userId: $scope.userId, weeklyMenu: weeklyMenu, consumers: 1, lang: $scope.config.language, settings: printSettings, date: null, author: null, headerInfo: null } }).then(function (response) { var fileName = response.data.d; $scope.pdfLink = '../upload/users/' + $scope.userId + '/pdf/' + fileName + '.pdf'; $scope.creatingPdf = false; }, function (response) { $scope.creatingPdf = false; alert(response.data.d) }); }
    $scope.print = function (weeklyMenu, printType) {
        if ($scope.creatingPdf) { return; }
        $scope.pdfLink = null; $scope.creatingPdf = true; $scope.currMenu = weeklyMenu.id; functions.post('PrintPdf', printType === 0 ? 'InitWeeklyMenuSettings' : 'InitMenuSettings', {}).then(function (d) { var printSettings = d; $scope.printType = printType; printWeeklyMenu(weeklyMenu, printSettings); });
    }
    $scope.hidePdfLink = function () { $scope.pdfLink = null; }
