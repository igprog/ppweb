angular.module('app', ['ui.router', 'pascalprecht.translate', 'chart.js', 'ngStorage', 'functions', 'charts']).config(['$stateProvider', '$urlRouterProvider', '$translateProvider', '$translatePartialLoaderProvider', '$httpProvider', function ($stateProvider, $urlRouterProvider, $translateProvider, $translatePartialLoaderProvider, $httpProvider) {
    ...
    $scope.today = new Date(); function initPrintSettings() { $http({ url: $sessionStorage.config.backend + 'PrintPdf.asmx/InitMenuSettings', method: "POST", data: {} }).then(function (response) { $scope.settings = JSON.parse(response.data.d); $scope.settings.showTotals = true; }, function (response) { alert(response.data.d) }); }; var saveVersion = function () {
    ...
    $scope.setLanguage = function (x) {
        ...
        $http({ url: $sessionStorage.config.backend + 'ClientApp.asmx/Activate', method: 'POST', data: { code: x } }).then(function (response) {
            $scope.clientApp = JSON.parse(response.data.d); if ($scope.clientApp.code === x) {
    ...
    var loadPals = function () { $http({ url: $sessionStorage.config.backend + 'Calculations.asmx/LoadPal', method: "POST", data: '' }).then(function (response) { $scope.pals = JSON.parse(response.data.d); }, function (response) { alert(response.data.d) }); }; var getConfig = function () {
    ...
    var getClientData = function () { $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/Get', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.clientData = JSON.parse(response.data.d); $scope.clientData.date = new Date(new Date().setHours(0, 0, 0, 0)); getActivitiesTotal($scope.clientData.activities); $scope.calculate(); }, function (response) { alert(response.data.d) }); }
    $scope.save = function (x) { x.date = functions.dateToString(x.date); $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/Save', method: "POST", data: { userId: $scope.userId, x: x, userType: 0 } }).then(function (response) { }, function (response) { alert(response.data.d) }); }
    ...
    var getCalculation = function () {
        ...
        $http({ url: $sessionStorage.config.backend + 'Calculations.asmx/GetCalculation', method: "POST", data: { client: $scope.clientData, userType: 1 } }).then(function (response) { $scope.calculation = JSON.parse(response.data.d); }, function (response) { alert(response.data.d) });
    }; $scope.calculate = function () { getCalculation(); }
    getConfig(); $scope.loading = false; $scope.loadMenues = function () { $scope.loading = true; $http({ url: $sessionStorage.config.backend + 'Menues.asmx/LoadClientMenues', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.menus = JSON.parse(response.data.d); angular.forEach($scope.menus, function (x, key) { var date_ = new Date(x.date); x.date = date_.toLocaleDateString(); }); $scope.loading = false; }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.getMenu = function (x) { $http({ url: $sessionStorage.config.backend + 'Menues.asmx/Get', method: "POST", data: { userId: $scope.userId, id: x.id, } }).then(function (response) { $scope.menu = JSON.parse(response.data.d); $scope.menu.client.clientData = $scope.clientData; getTotals($scope.menu); }, function (response) { alert(response.data.d); }); }
    ...
    var getTotals = function (x) { $http({ url: $sessionStorage.config.backend + 'Foods.asmx/GetTotals', method: "POST", data: { selectedFoods: x.data.selectedFoods, meals: x.data.meals } }).then(function (response) { $scope.totals = JSON.parse(response.data.d); $scope.totals.price.currency = $scope.config.currency; }, function (response) { alert(response.data.d) }); }
    var consumers = 1; $scope.rowsPerPage = 51; $scope.pdfLink = null; $scope.creatingPdf = false; $scope.createMenuPdf = function (rowsPerPage) { $scope.pdfLink = null; $scope.creatingPdf = true; $http({ url: $sessionStorage.config.backend + 'PrintPdf.asmx/MenuPdf', method: "POST", data: { userId: $scope.userId, currentMenu: $scope.menu, totals: $scope.totals, consumers: consumers, lang: $scope.config.language, settings: $scope.settings, date: null, author: null, headerInfo: null, rowsPerPage: rowsPerPage } }).then(function (response) { var fileName = response.data.d; $scope.creatingPdf = false; $scope.pdfLink = $sessionStorage.config.backend + 'upload/users/' + $scope.userId + '/pdf/' + fileName + '.pdf'; }, function (response) { $scope.creatingPdf = false; alert(response.data.d) }); }
    ...
    var updateClient = function (x) { var c = angular.copy(x); c.birthDate = functions.dateToString(c.birthDate); $http({ url: $sessionStorage.config.backend + 'Clients.asmx/UpdateClient', method: 'POST', data: { userId: $scope.userId, x: c } }).then(function (response) { document.getElementById("mySidenav").style.width = "0"; }, function (response) { alert(response.data.d); }); }
    ...
    var getClientData = function () { $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/Get', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.clientData = JSON.parse(response.data.d); $scope.clientData.date = new Date(new Date().setHours(0, 0, 0, 0)); getCalculation(); getClientLog(); }, function (response) { alert(response.data.d) }); }
    var getCalculation = function () {
        if (isNaN($scope.clientData.weight) == true || isNaN($scope.clientData.height) == true || isNaN($scope.clientData.waist) == true || isNaN($scope.clientData.hip) == true) { return false; }
        $http({ url: $sessionStorage.config.backend + 'Calculations.asmx/GetCalculation', method: "POST", data: { client: $scope.clientData, userType: 1 } }).then(function (response) { $scope.calculation = JSON.parse(response.data.d); }, function (response) { alert(response.data.d) });
    }; var getClientLog = function () { $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/GetClientLog', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.clientLog = JSON.parse(response.data.d); setClientLogGraphData(0, $scope.clientLogsDays); }, function (response) { alert(response.data.d) }); }
    ...
    $scope.displayType = 0; var getClientLog = function () { $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/GetClientLog', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.clientLog = JSON.parse(response.data.d); angular.forEach($scope.clientLog, function (x, key) { x.date = new Date(x.date); }); setClientLogGraphData(0, $scope.clientLogsDays); }, function (response) { alert(response.data.d) }); }
    getClientLog(); $scope.updateClientLog = function (x) { var cd = angular.copy(x); cd.date = functions.dateToString(cd.date); $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/UpdateClientLog', method: "POST", data: { userId: $scope.userId, clientData: cd } }).then(function (response) { getClientLog(); }, function (response) { alert(response.data.d) }); }
    ...
    var removeClientLog = function (x) { $http({ url: $sessionStorage.config.backend + 'ClientsData.asmx/Delete', method: "POST", data: { userId: $scope.userId, clientData: x } }).then(function (response) { getClientLog(); }, function (response) { alert(response.data.d) }); }
