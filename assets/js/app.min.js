angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    $scope.loadTickets = function () { $scope.activeTab = 'tickets'; $scope.loading = true; functions.post('Tickets', 'Load', {}).then(function (d) { $scope.tickets = d; $scope.currIdx = null; $scope.loading = false; }); }
    $scope.saveTicket = function (x, sendMail) { functions.post('Tickets', 'Save', { x, sendMail: sendMail, lang: 'hr' }).then(function (d) { $scope.loadTickets(); }); }
    $scope.removeTicket = function (x) { if (confirm("Briši ticket: " + x.title + " (" + x.desc + ")?")) { functions.post('Tickets', 'Delete', { id: x.id }).then(function (d) { $scope.loadTickets(); }); } }
