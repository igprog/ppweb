angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    var getConfig = function () { $http.get('./config/config.json').then(function (response) { $rootScope.config = response.data; }); }; getConfig(); $scope.islogin = false; var d = new Date(); $scope.year = d.getFullYear(); $scope.toggleTpl = function (x) { $rootScope.tpl = x; }; $scope.toggleTpl('login'); init = function () { $scope.user = { username: '', password: '' } }
    init(); $scope.login = function (u) { $http({ url: $rootScope.config.backend + 'Admin.asmx/Login', method: 'POST', data: { username: u.username, password: u.password } }).then(function (response) { $scope.islogin = JSON.parse(response.data.d); if ($scope.islogin == true) { $scope.toggleTpl('programPrehraneWeb'); } else { alert('error login'); } }, function (response) { $scope.islogin = false; alert(response.data.d); }); }
    $scope.logout = function () { $scope.islogin = false; $scope.toggleTpl('login'); init(); }
}]).controller('applicationCtrl', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
    var load = function () { $http({ url: $rootScope.config.backend + 'Instal.asmx/Load', method: 'POST', data: '' }).then(function (response) { $scope.d = JSON.parse(response.data.d); }, function (response) { alert(response.data.d); }); }
    load();
}]).controller('webAppCtrl', ['$scope', '$http', '$rootScope', 'functions', function ($scope, $http, $rootScope, functions) {
    $scope.showDetails = false; $scope.showActive = false; $scope.loading = false; $scope.limit = 10; $scope.page = 1; $scope.searchQuery = ''; $scope.showUsers = true; function setYears() {
        $scope.years = []; for (var i = 2017; i <= $scope.year; i++) { $scope.years.push(i); }
        $scope.year = new Date().getFullYear();
    }
    setYears(); var total = function (year) { $scope.loading = true; $http({ url: $rootScope.config.backend + 'Users.asmx/Total', method: 'POST', data: { year: year } }).then(function (response) { $scope.t = JSON.parse(response.data.d); $scope.loading = false; }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.total = function (year) { $scope.showUsers = false; google.charts.load('current', { packages: ['corechart'] }); total(year); }
    function drawChart() {
        $scope.loadingChart = true; var data = new google.visualization.DataTable(); data.addColumn('string', 'Mjesec'); data.addColumn('number', 'Registracije'); data.addColumn('number', 'Aktivacije'); data.addColumn('number', 'Postotak'); var tl = $scope.t.monthly; angular.forEach(tl, function (value, key) { data.addRows([[value.month, value.registration, value.activation, value.percentage]]); })
        var options = { chart: { title: 'Pregled registracija i aktivacija' }, height: (tl.length * 55) + 2, chartArea: { height: tl.length * 55, width: 350 } }; var chart = new google.visualization.BarChart(document.getElementById('chart_ppweb')); chart.draw(data, options); $scope.loadingChart = false;
    }
    var load = function (limit) { $scope.loading = true; $http({ url: $rootScope.config.backend + 'Users.asmx/Load', method: 'POST', data: { limit: limit, page: $scope.page } }).then(function (response) { $scope.loading = false; $scope.d = JSON.parse(response.data.d); }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.search = function (searchQuery, showActive, limit) { $scope.loading = true; $scope.showUsers = true; $scope.page = 1; $http({ url: $rootScope.config.backend + 'Users.asmx/Search', method: 'POST', data: { query: searchQuery, limit: limit, page: $scope.page, activeUsers: showActive } }).then(function (response) { $scope.d = JSON.parse(response.data.d); $scope.loading = false; }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.search(null, false, $scope.limit); $scope.update = function (user) { $http({ url: $rootScope.config.backend + 'Users.asmx/Update', method: 'POST', data: { x: user } }).then(function (response) { load($scope.limit); total($scope.year); alert(response.data.d); }, function (response) { alert(response.data.d); }); }
    $scope.currUser = null; $scope.info = function (x) { $scope.currUser = x; $http({ url: $rootScope.config.backend + 'Users.asmx/GetUserSum', method: 'POST', data: { userGroupId: x.userGroupId, userId: x.userId, userType: x.userType, adminType: x.adminType } }).then(function (response) { $scope.userTotal = JSON.parse(response.data.d); }, function (response) { alert(response.data.d); }); }
    $scope.remove = function (user) { var r = confirm("Briši " + user.firstName + " " + user.lastName + "?"); if (r == true) { remove(user); } }
    var remove = function (user) { $http({ url: $rootScope.config.backend + 'Users.asmx/DeleteAllUserGroup', method: 'POST', data: { x: user } }).then(function (response) { load($scope.limit); total($scope.year); alert(response.data.d); }, function (response) { alert(response.data.d); }); }
    ...
    $scope.updateInfo = function (x) { $http({ url: $rootScope.config.backend + 'Users.asmx/UpdateUserInfoFromOrdersTbl', method: 'POST', data: { email: x } }).then(function (response) { alert(response.data.d); }, function (response) { alert(response.data.d); }); }
    ...
    var load = function () { functions.post('Orders', 'Load', {}).then(function (d) { $scope.orders = d; }); }
    load(); $scope.createInvoice = function (order, tpl) { functions.post('Invoice', 'InitPP', { order: order }).then(function (d) { $rootScope.i = d; $rootScope.tpl = tpl; }); }
    ...
    var initForm = function () { $scope.isInvoice = false; $scope.pdfTempLink = null; $scope.pdfLink = null; $scope.loading = false; $scope.loading_1 = false; $scope.loading_2 = false; $scope.invoices = []; $scope.showInvoices = false; $scope.total = angular.isDefined($rootScope.i) ? $scope.getTotal($rootScope.i.items) : 0; $scope.year = new Date().getFullYear(); $scope.isForeign = false; $scope.clientLeftSpacing = 300; $scope.isOffer = false; }
    initForm(); $scope.init = function () { $scope.showInvoices = false; $http({ url: $rootScope.config.backend + 'Invoice.asmx/Init', method: 'POST', data: '' }).then(function (response) { $rootScope.i = JSON.parse(response.data.d); $rootScope.i.dateAndTime = getLocalDateAndTime(); initForm(); }, function (response) { alert(response.data.d); }); }
    ...
    $scope.totPrice_eur = 0; $scope.createPdf = function (i, isForeign, totPrice_eur, clientLeftSpacing, isOffer) {
