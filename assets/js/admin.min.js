angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    $scope.showDetails = false; $scope.showActive = false; $scope.loading = false; $scope.limit = 10; $scope.page = 1; $scope.searchQuery = ''; $scope.isDesc = true; $scope.activeTab = 'users'; function setYears() {
    ...
    var remove = function (user) { if (user.userId !== user.userGroupId) { alert('Samo glavni nositelj računa može biti obrisan.'); } else { functions.post('Users', 'DeleteAllUserGroup', { x: user }).then(function (d) { load($scope.limit); alert(d); }); } }
    ...
    $scope.loadSharingRecipes = function () { $scope.activeTab = 'sharingRecipes'; $scope.loading = true; functions.post('SharingRecipes', 'Load', { userId: null, status: null, showUserRecipes: true }).then(function (d) { $scope.sharingRecipes = d; $scope.loading = false; }); }
    $scope.GetSharingRecipe = function (x, idx) { $scope.loading = true; functions.post('SharingRecipes', 'Get', { id: x.recipe.id }).then(function (d) { $scope.sharingRecipes[idx] = d; $scope.loading = false; }); }
    $scope.saveSharingRecipe = function (x) { x.adminSave = true; functions.post('SharingRecipes', 'Save', { x: x }).then(function (d) { $scope.loadSharingRecipes(); }); }
    ...
    $scope.saveErrorLog = function (x) { functions.post('Log', 'Save', { fileName: 'error.log', content: x }).then(function (d) { }); }
