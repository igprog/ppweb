angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    $scope.showDetails = false; $scope.showActive = false; $scope.loading = false; $scope.limit = 10; $scope.page = 1; $scope.searchQuery = ''; $scope.isDesc = true; $scope.activeTab = 'users'; $scope.adminCode = null; $scope.licenceStatus = null; function setYears() {
    ...
    $scope.search = function (searchQuery, showActive, licenceStatus, limit, isDesc) { $scope.loading = true; $scope.activeTab = 'users'; $scope.page = 1; $scope.isDesc = isDesc; functions.post('Users', 'Search', { query: searchQuery, limit: limit, page: $scope.page, activeUsers: showActive, licenceStatus: licenceStatus, isDesc: isDesc }).then(function (d) { $scope.d = d; $scope.loading = false; }); }
    $scope.search(null, false, null, $scope.limit, $scope.isDesc); $scope.update = function (user) { functions.post('Users', 'Update', { x: user }).then(function (d) { load($scope.limit); alert(d); }); }
    ...
    $scope.remove = function (user) { if (confirm("Briši " + user.firstName + " " + user.lastName + "?")) { remove(user); } }
    var remove = function (user) {
        if (user.userId !== user.userGroupId) { alert('Samo glavni nositelj računa može biti obrisan.'); return; }
        if (user.licenceStatus === 'active') { alert('Aktivan korisnik ne može biti izvrisan. Ako želiš izbrisati aktivnog korisnika prvo ga deaktiviraj.'); return; }
        functions.post('Users', 'DeleteAllUserGroup', { x: user }).then(function (d) { load($scope.limit); alert(d.msg); });
    }
    ...
    var loadSettings = function () { functions.post('Files', 'LoadSettings', {}).then(function (d) { $scope.settings = d; }); }
    loadSettings(); $scope.activeTabActivity = 'lastActivity'; $scope.loadActivityLog = function () { $scope.activeTab = 'activityLog'; $scope.loading = true; functions.post('Log', 'Load', { fileName: 'activity.log' }).then(function (d) { $scope.activityLog = d; loadLoginLog(false); $scope.loading = false; }); }
    var loadLoginLog = function (isAll) { $scope.loading = true; functions.post('Log', 'LoadLoginLog', { isAll: isAll }).then(function (d) { $scope.loginLog = d; $scope.loading = false; }); }
    $scope.loadLoginLog = function (isAll) { $scope.activeTabActivity = isAll ? 'allActivities' : 'lastActivity'; loadLoginLog(isAll); }
    ...
    $scope.removeInvoice = function (x, year, search) { if (confirm("Briši račun br." + x.number + ", Iznos: " + x.total + "?")) { $http({ url: $rootScope.config.backend + 'Invoice.asmx/Delete', method: 'POST', data: { x: x } }).then(function (response) { $scope.load(year, search); alert(response.data.d); }, function (response) { alert(response.data.d); }); } }
