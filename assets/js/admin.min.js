angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    $scope.showDetails = false; $scope.showActive = false; $scope.loading = false; $scope.limit = 10; $scope.page = 1; $scope.searchQuery = ''; $scope.showUsers = true; $scope.isDesc = true; function setYears() {
    ...
    var load = function (limit) { $scope.loading = true; $http({ url: $rootScope.config.backend + 'Users.asmx/Load', method: 'POST', data: { limit: limit, page: $scope.page, isDesc: $scope.isDesc } }).then(function (response) { $scope.loading = false; $scope.d = JSON.parse(response.data.d); }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.search = function (searchQuery, showActive, limit, isDesc) { $scope.loading = true; $scope.showUsers = true; $scope.page = 1; $scope.isDesc = isDesc; $http({ url: $rootScope.config.backend + 'Users.asmx/Search', method: 'POST', data: { query: searchQuery, limit: limit, page: $scope.page, activeUsers: showActive, isDesc: isDesc } }).then(function (response) { $scope.d = JSON.parse(response.data.d); $scope.loading = false; }, function (response) { $scope.loading = false; alert(response.data.d); }); }
    $scope.search(null, false, $scope.limit, $scope.isDesc); $scope.update = function (user) { $http({ url: $rootScope.config.backend + 'Users.asmx/Update', method: 'POST', data: { x: user } }).then(function (response) { load($scope.limit); alert(response.data.d); }, function (response) { alert(response.data.d); }); }
    ...
    $scope.remove = function (x) { if (confirm("Briši narudžbu br. " + x.orderNumber + " (" + x.firstName + " " + x.lastName + " - " + x.companyName + ")?")) { $http({ url: $rootScope.config.backend + 'Orders.asmx/Delete', method: 'POST', data: { id: x.id } }).then(function (response) { alert(response.data.d); load(); }, function (response) { alert(response.data.d); }); } }
