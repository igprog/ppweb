angular.module('app', []).config(['$httpProvider', function ($httpProvider) {
    ...
    load(); $scope.pp5DownloadEnableCode = null; var getPp5DownloadEnableCode = function (x) { functions.post('Files', 'GetTempFile', { fileName: 'pp5DownloadEnableCode.txt' }).then(function (d) { $scope.pp5DownloadEnableCode = d; }); }; getPp5DownloadEnableCode(); $scope.confirmPp5DownloadEnableCode = function (x) { functions.post('Files', 'SaveTempFilePP5', { fileName: 'pp5DownloadEnableCode.txt', content: x }).then(function (d) { alert(d); }); };
    ...
    $scope.currIdx = null; $scope.showTicketDetails = function (idx) { $scope.currIdx = idx; }
    $scope.loadTickets = function () { $scope.activeTab = 'tickets'; $scope.loading = true; functions.post('Tickets', 'Load', {}).then(function (d) { $scope.tickets = d; $scope.loading = false; }); }
    $scope.saveTicket = function (x, sendMail) { functions.post('Tickets', 'Save', { x, sendMail: sendMail, lang: 'hr' }).then(function (d) { alert(d.response.msg); }); }
    $scope.removeTicket = function (x) { if (confirm("Briši ticket: " + x.title + " (" + x.desc + ")?")) { functions.post('Tickets', 'Delete', { id: x.id }).then(function (d) { $scope.loadTickets(); alert(d); }); } }
